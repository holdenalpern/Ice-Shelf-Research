...
def sum_sq_err(data, pred):
    """
    Calculate the sum of squared errors between data and predictions.
    """
    return np.sum(np.square(data - pred))

def mcmc_rbf_blocked_prism(ds, x0, pred_coords, target, sigma, pgen, Sigma, bsize, var, rng, weights, iter_num=1000, stop=None, quiet=False, save_path=None):
    """
    MCMC for bathymetry inference.
    
    Parameters:
    - ds: xarray Dataset containing grid information and masks.
    - x0: Initial bed elevation array.
    - pred_coords: Tuple of (x_obs, y_obs, height_obs) coordinates.
    - target: 2D NumPy array representing target gravity data.
    - sigma: Gravity uncertainty.
    - pgen: PrismGen instance for creating prisms.
    - Sigma: Covariance matrix for block updates.
    - bsize: Block size for updates.
    - var: Variance scaling factor.
    - rng: NumPy random number generator.
    - weights: Optional weights for block updates.
    - iter_num: Number of MCMC iterations.
    - stop: RMSE stopping condition.
    - quiet: Boolean to disable progress bar.
    - save_path: Path to save intermediate results.
    
    Returns:
    - result: Dictionary containing bed, loss, step, and gravity caches.
    """
    
    bed = x0.copy()
    ny, nx = bed.shape
    te_dist = target  # Assuming 'target' is a 2D grid (ny, nx)
    
    # Create mask based on 'mask' and 'inv_mask'
    shelf_msk = np.where((ds.mask == 3) & (ds.inv_mask == True), True, False)
    
    # Initialize caches
    bed_cache = np.zeros((iter_num, ny, nx))
    loss_cache = np.zeros(iter_num)
    step_cache = np.zeros(iter_num, dtype=bool)
    grav_cache = np.zeros((iter_num, target.size))
    ...
    # Initialize loss
    prisms, densities = pgen.make_prisms(ds, bed, msk='inv', ice=False)
    g_z = hm.prism_gravity(pred_coords, prisms, densities, field='g_z', parallel=False)  # Adjust 'parallel' as needed
    ...
    for i in pbar:
        ...
        if weights is None:
            bed_next, idxs_next = block_update_sample(bed, ds.inv_mask.data, bsize, samples[i], rng)
        else:
            bed_next, idxs_next = block_update_sample(bed, ds.inv_mask.data, bsize, samples[i], rng, weights)
    
        # Ensure bed is below ice bottom
        bed_next = np.where(bed_next > ds.surface - ds.thickness, ds.surface - ds.thickness, bed_next)
        
        # Trim dataset to block area
        i_start, i_end, j_start, j_end = idxs_next  # Assuming idxs_next is [i_start, i_end, j_start, j_end]
        i_msk = np.full(ds.y.size, False)
        i_msk[i_start:i_end] = True
        j_msk = np.full(ds.x.size, False)
        j_msk[j_start:j_end] = True
        ds_trim = ds.isel(x=j_msk, y=i_msk)
    
        bed_trim = bed[i_start:i_end, j_start:j_end]
        bed_next_trim = bed_next[i_start:i_end, j_start:j_end]
        ...
        pred_coords_trim = (
            pred_coords[0][coords_ind],
            pred_coords[1][coords_ind],
            pred_coords[2][coords_ind]
        )
        x_obs_trim, y_obs_trim, height_obs_trim = pred_coords_trim
    
        # Calculate gravity for previous bed_trim
        prisms_prev, densities_prev = pgen.make_prisms(ds_trim, bed_trim, msk='inv', ice=False)
        g_z_prev_block = hm.prism_gravity(
            (x_obs_trim, y_obs_trim, height_obs_trim),
            prisms_prev,
            densities_prev,
            field='g_z',
            parallel=False  # Adjust as needed
        )
        
        # Calculate gravity for new bed_next_trim
        prisms_next, densities_next = pgen.make_prisms(ds_trim, bed_next_trim, msk='inv', ice=False)
        g_z_next_block = hm.prism_gravity(
            (x_obs_trim, y_obs_trim, height_obs_trim),
            prisms_next,
            densities_next,
            field='g_z',
        ...
        g_z_change[coords_ind] = -g_z_prev_block + g_z_next_block
        ...
        # Save current bed and gravity
        bed_cache[i, :, :] = bed
        grav_cache[i, :] = g_z.flatten()
    
        # Optionally save progress
        if (i > 0) and (i % 10_000 == 0) and (save_path is not None):
            np.savez_compressed(
                save_path, 
        ...
        current_rmse = np.sqrt(loss_cache[i] / target.size)
        pbar.set_description(f'Iter {i+1}/{iter_num} RMSE: {current_rmse:.3f}')
